...
echoLn(touchCounter());
echoLn(touchCounter());
echoLn("----------END EXAMPLE 10.4 ----------------");

//---------------------------------

function touchCounter2() {
    if (isset($a)) {
        echoLn("a exists before static statment");
    }

    static $a = 0;

    if (isset($a)) {
        echoLn("a exists after static statment");
    }

    return $a++;
}

echoLn(touchCounter2());
echoLn(touchCounter2());
echoLn(touchCounter2());
echoLn("----------END EXAMPLE 10.5 ----------------");
...
function printValues(int $integer, float $float, string $string): bool {
...
$func = function($count) use($symbol, &$counter) {
    while($count > 0) {
        $counter++;
        echo "line $counter" . $symbol;
        $count--;
    }
...
echo "check symbol: \n value" . PHP_EOL;
echo "check symbol: $symbol value" . PHP_EOL;
echo "check counter: $counter value" . PHP_EOL;
echo "check counter: 5 value" . PHP_EOL;
echoLn("----------END EXAMPLE 15.3 ----------------");
